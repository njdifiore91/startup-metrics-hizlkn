...
describe('BenchmarkChart Component', () => {
  ...
  beforeEach(() => {
    ...
    vi.clearAllMocks();
    (Chart as unknown as Mock).mockClear();
  ...
  it('updates chart when company metric is provided', async () => {
    ...
    await waitFor(() => {
      const chartInstance = (Chart as unknown as Mock).mock.instances[0];
  ...
  it('memoizes component for performance', () => {
    const { rerender } = render(<BenchmarkChart {...defaultProps} />);
    const initialRenderCount = (Chart as unknown as Mock).mock.calls.length;

    // Rerender with same props
    rerender(<BenchmarkChart {...defaultProps} />);

    expect((Chart as unknown as Mock).mock.calls.length).toBe(initialRenderCount);
  });
});
