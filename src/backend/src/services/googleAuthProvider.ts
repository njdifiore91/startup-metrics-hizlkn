...
export class GoogleAuthProvider implements IAuthProvider {
  ...
  async validateToken(token: string): Promise<IUser> {
    try {
      ...
      throw new AppError('Token validation failed', HTTP_STATUS.UNAUTHORIZED, AUTH_ERRORS.INVALID_TOKEN.code);
    }
  }
  ...
  async exchangeCode(code: string, redirectUri: string): Promise<IUser> {
    try {
      ...
      throw new AppError('Code exchange failed', HTTP_STATUS.UNAUTHORIZED, AUTH_ERRORS.INVALID_TOKEN.code);
    }
  }
  ...
  async revokeToken(token: string): Promise<void> {
    try {
      ...
      // Don't throw error on revocation failure
    }
  }
